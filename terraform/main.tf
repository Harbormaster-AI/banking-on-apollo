...
provider "aws" {
  ...
  default_tags {
    tags = {
      ...
      CertificationId = "5bd22708-0275-442e-b29f-74f5a6e77de7"
    }
  }
}
...
resource "aws_key_pair" "generated" {
  key_name   = "pjsk-sshtest-0.13069049681209233"
...
resource "aws_db_subnet_group" "default" {
  name       = "bankingonapollo-db-subnet"
...
#  name        = "bankingonapollo-security-group-from-terraform" #optional, when omitted, terraform creates a random name
  description = "security group for application bankingonapollo created from terraform"
  vpc_id      = aws_vpc.default.id
...
resource "aws_security_group" "db" {
  description = "security group for bankingonapollo and mysql created from terraform"
  vpc_id      = aws_vpc.default.id
...
#  identifier             = "bankingonapollo-rds" # Terraform will create a unique id if not assigned
  allocated_storage      = 20
  engine                 = "mysql"
  instance_class         = "db.t3.medium"
  db_name                = "bankingonapollo"
...
resource "aws_eks_cluster" "this" {
  name     = "eks_cluster_bankingonapollo"
...
resource "aws_instance" "web" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "sudo docker pull theharbormaster/banking-on-apollo:latest",
      "sudo docker run -d -p 8000:8000 -p 8080:8080 -e DATABASE_URL=jdbc:mysql://${aws_db_instance.default.endpoint}/bankingonapollo theharbormaster/banking-on-apollo:latest"
    ]
  }
}
